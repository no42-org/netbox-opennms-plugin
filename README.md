...
1. Build and push an image with the plugin. Match the NetBox base to the chart's `appVersion` (4.6.1 or later) and pin both versions:
   ...
   RUN /opt/netbox/venv/bin/pip install netbox-opennms-plugin==0.0.13
   ```

   ```bash
   docker build -t registry.example.org/netbox-opennms:v4.7.1-0.0.13 .
   docker push registry.example.org/netbox-opennms:v4.7.1-0.0.13
   ```
